...
class PendingAgentActionService
{
    ...
    {
        return PendingAgentAction::query()->firstOrCreate(['idempotency_key' => $key], ['user_id' => $user->id, 'agent_conversation_id' => $conversationId, 'tool_name' => $tool, 'payload' => $payload, 'missing_fields' => $missing, 'status' => 'pending']);
    }
    ...
    public function confirm(PendingAgentAction $action, User $user, AgentToolExecutor $executor): PendingAgentAction
    {
        ...
        $result = $executor->execute($action->tool_name, $action->payload, $user, true);
