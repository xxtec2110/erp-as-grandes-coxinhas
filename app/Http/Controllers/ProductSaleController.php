...
class ProductSaleController extends Controller
{
    public function index(Request $request, AuthorizationService $auth, SalesSummaryService $summary): View
    {
        ...
        return view('sales.index', ['sales' => ProductSale::query()->with(['product.category', 'location', 'creator', 'acquirer', 'cardBrand', 'saleOrder.payments'])->whereNull('cancelled_at')->when($location, fn ($query) => $query->where('location_id', $location->id))->when($paymentMethod, fn ($query) => $query->where(fn ($query) => $query->where('payment_method', $paymentMethod)->orWhereHas('paymentAllocations.payment', fn ($query) => $query->where('payment_method', $paymentMethod))))->latest('operation_date')->paginate(20)->withQueryString(), 'locations' => $locations, 'location' => $location, 'startDate' => $start, 'endDate' => $end, 'paymentMethod' => $paymentMethod, 'paymentMethods' => ProductSalePaymentMethod::cases(), 'summary' => $location ? $summary->summarize($location, $start, $end, $paymentMethod) : []]);
    }
