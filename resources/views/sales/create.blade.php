...
<label><span class="form-label">Forma de pagamento</span><select class="form-input" name="payment_method" required>@foreach($paymentMethods as $method)<option value="{{ $method->value }}" @selected(old('payment_method','cash')===$method->value)>{{ $method->label() }}{{ $method->requiresCardConfiguration() ? '' : ' / sem taxa de cartão' }}</option>@endforeach</select><span class="mt-1 block text-xs text-stone-500">Pix é uma forma própria e não exige adquirente ou bandeira.</span></label>
