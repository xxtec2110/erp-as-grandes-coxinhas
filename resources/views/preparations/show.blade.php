...
@section('content')
    ...
    @if (! $calculation['is_complete'])<div class="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900"><strong>Cálculo incompleto.</strong> Não há custo final enquanto faltarem dados reais. @if($calculation['missing_components'] !== []) Pendências: {{ implode('; ', $calculation['missing_components']) }}. @endif</div>@endif

    <section class="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="metric-card border-amber-300 bg-amber-50"><p class="metric-label">{{ $calculation['is_complete'] ? 'Custo total do preparo' : 'Subtotal conhecido' }}</p><p class="metric-value text-xl">R$ {{ \App\Support\DecimalFormatter::format($calculation['total_preparation_cost']) }}</p></div>
