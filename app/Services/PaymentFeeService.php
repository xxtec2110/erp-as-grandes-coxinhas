...
class PaymentFeeService
{
    ...
    public function apply(array $data, User $user, string $source = 'web', ?int $importId = null): PaymentFee
    {
        return DB::transaction(function () use ($data, $user, $source, $importId): PaymentFee {
            $query = PaymentFee::query()->where('acquirer_id', $data['acquirer_id'])->where('card_brand_id', $data['card_brand_id'])->where('payment_method', $data['payment_method'])->where(fn ($query) => empty($data['installments']) ? $query->whereNull('installments') : $query->where('installments', $data['installments']));
            ...
            $fee = PaymentFee::query()->create([...$data, 'installments' => $data['installments'] ?: null, 'is_current' => true, 'active' => true, 'source' => $source, 'created_by' => $user->id, 'payment_fee_import_id' => $importId]);
