...
class PdvIntegrationEventService
{
    ...
    {
        PdvIntegrationEvent::query()->create(['pdv_connection_id' => $connection?->id, 'pdv_inbound_event_id' => $inbound?->id, 'user_id' => $user?->id, 'event_type' => $type, 'status' => $status, 'duration_ms' => $durationMs, 'lag_seconds' => $lagSeconds, 'metadata' => $metadata]);
    }
}
