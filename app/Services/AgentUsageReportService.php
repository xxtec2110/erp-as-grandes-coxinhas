...
class AgentUsageReportService
{
    public function summarize(CarbonImmutable $start, CarbonImmutable $end)
    {
        ...
        $costs = AgentUsageCost::query()->whereBetween('created_at', [$start, $end])->selectRaw('user_id, SUM(input_tokens) input_tokens, SUM(output_tokens) output_tokens, SUM(estimated_cost) total_cost')->groupBy('user_id')->get()->keyBy('user_id');
