                    ...
                    <article class="rounded-xl border bg-white p-5 shadow-sm">
                        ...
                        @else
                            <form class="mt-4 space-y-3" method="POST" action="{{ route('pdv.mappings.payments.update', [$connection, $row['external_form_id']]) }}">
                                ...
                                <p class="rounded-lg bg-stone-50 p-3 text-sm"><strong>Representação ERP:</strong> {{ $row['compatibility']['label'] }}<span class="block text-xs text-stone-500">{{ $row['compatibility']['requires_rate'] ? 'Exige adquirente e taxa vigente. Bandeira é opcional porque o contrato atual do GrandChef não a fornece.' : 'Não exige adquirente, bandeira ou taxa.' }}</span></p>
                                @if ($row['compatibility']['requires_acquirer'])
                                    @if ($row['configuration_missing'])
                                        <div class="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-900"><strong>Configuração financeira necessária antes de confirmar.</strong><span class="mt-1 block">Cadastre uma taxa vigente para uma adquirente; use bandeira somente quando a regra humana realmente se aplicar.</span><a class="mt-2 inline-flex font-bold underline" href="{{ route('payment-fees.batch') }}">Configurar taxas</a></div>
                                    @else
                                        <label class="text-sm font-semibold">Configuração financeira existente<select class="mt-1 w-full rounded-lg border-stone-300" name="financial_configuration" required><option value="">Selecione</option>@foreach ($row['financial_options'] as $fee)<option value="{{ $fee->acquirer_id }}:{{ $fee->card_brand_id ?? 'none' }}" @selected($row['mapping']?->acquirer_id === $fee->acquirer_id && $row['mapping']?->card_brand_id === $fee->card_brand_id)>{{ $fee->acquirer->name }} · {{ $fee->cardBrand?->name ?? 'sem bandeira externa' }} · {{ \App\Support\DecimalFormatter::format($fee->fee_percentage, 4) }}% + R$ {{ \App\Support\DecimalFormatter::format($fee->fixed_fee, 2) }}</option>@endforeach</select></label>
                                    @endif
                                @endif
                                @if ($row['mapping_status'] === 'confirmed')
