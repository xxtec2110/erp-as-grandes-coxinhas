...
class UserAccessController extends Controller
{
    ...
    public function edit(User $user, AuthorizationService $authorization, DashboardVisibilityService $dashboardVisibility, DashboardWidgetRegistry $dashboardRegistry): View
    {
        ...
        return view('users.access', ['managedUser' => $user->load(['roles', 'permissions', 'locations']), 'roles' => Role::query()->orderBy('label')->get(), 'permissions' => Permission::query()->orderBy('group')->orderBy('label')->get(), 'locations' => Location::query()->orderBy('name')->get(), 'effective' => $authorization->effectivePermissions($user), 'canManageDashboard' => $canManageDashboard, 'dashboardWidgets' => $canManageDashboard ? $dashboardVisibility->configuration($user) : collect(), 'dashboardGroups' => $dashboardRegistry->groups()]);
    }
